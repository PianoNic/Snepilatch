...
**Nightlies are cut on a schedule**: the **"Cut a Nightly Build"** workflow (`nightly.yml`) runs twice a day (cron, UTC — see the workflow for the exact times) plus `workflow_dispatch` for a manual cut. It skips entirely if `main` hasn't moved since the last nightly tag, so a quiet stretch produces nothing. Otherwise it reads the current `versionName` out of `build.gradle.kts` as the base, finds the next free `-nightly.N` suffix for that base (scanning existing tags), tags and pushes it, publishes it as a GitHub **prerelease**, then explicitly dispatches `build-and-release.yml` at that tag. It can't just rely on the `release: published` event the way a stable release does — a release published via the workflow's own `GITHUB_TOKEN` does not fire that event (GitHub's anti-recursion safeguard against workflows retriggering themselves), so `workflow_dispatch` (which is exempt) is used instead. A `concurrency` group serializes runs so a manual dispatch landing close to a scheduled run can't race it.
